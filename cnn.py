...
class CNN:
    ...
    #컨볼루션 층을 만듦
    def convolution_layer(self, pre_output, x, y, depth, num_of_filter, move_right, move_down):
        # 필터를 32개 만듦 : 3x3x1 짜리 필터
        # L1 ImgIn shape=(?, 28, 28, 1)
        W = tf.Variable(tf.random_normal([x, y, depth, num_of_filter], stddev=0.01)) # 3x3x1 필터 32개
        #    Conv     -> (?, 28, 28, 32)
        #    Pool     -> (?, 14, 14, 32)
    ...
    def output_layer(self, pre_layer, input_size, output_size, w_name):
        reshaped_input = tf.reshape(pre_layer, [-1, 7*7* 64])

        # Final FC 7x7x64 inputs -> 10 outputs
        W = tf.get_variable(w_name, shape=[input_size, output_size], initializer=tf.contrib.layers.xavier_initializer())
        b = tf.Variable(tf.random_normal([output_size]))
        hypothesis = tf.matmul(reshaped_input, W) + b
        return hypothesis

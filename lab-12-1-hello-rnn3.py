...
class XXX:
    ...
    def learn(self, xonehot, ydata):
        ...
        self.set_cost_function(batch_size=1, seq_len=1)
        ...
        for i in range(100):
            for x, y in zip(xonehot, ydata):
                x2 = [[x]]
                y2 = [[y]]
                #print(x2, y2)

                #print('shape of x and y:', , )
                err, _ = self.sess.run([self.cost_function, self.optimizer], feed_dict={self.X: x_one_hot, self.Y: y_data})
    ...
    def what_is_next(self, char_data):
        ...
        print("My suggestion is ", idx2char[result[0][0]], '\n')
...
y_data = [1, 0, 2, 3, 3, 4]  # hihello  (6)



gildong = XXX()
gildong.learn(x_one_hot, y_data)
gildong.what_is_next([[[1, 0, 0, 0, 0]]])
gildong.what_is_next([[[0, 1, 0, 0, 0]]])
gildong.what_is_next([[[1, 0, 0, 0, 0]]])
gildong.what_is_next([[[0, 0, 1, 0, 0]]])
gildong.what_is_next([[[0, 0, 0, 1, 0]]])
gildong.what_is_next([[[0, 0, 0, 1, 0]]])
